...
do 
    cw=$(echo $i | awk '{ printf "%.1f" ,$1*0.1}')
    #nohup python scripts/plan_cost_guided.py --dataset $1 --control_interval 2 --n_test_episode $n_test_episode --gpu $gpu_id --guide_type $guide_type --value_loadpath "$value_loadpath" --cost_value_loadpath "$cost_value_loadpath" --discount 0.99 --seed $seed --ratio_of_maxthreshold $cw --cost_grad_weight $cost_grad_weight --batch_size $batch_size > ./results/$1/cost/vel_cost_discount_cw${cost_grad_weight}_seed${seed}_guide_type${guide_type}_${cw}.txt &
