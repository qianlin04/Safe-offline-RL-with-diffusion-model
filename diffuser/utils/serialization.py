...
def load_decouple_diffusion(*loadpath, epoch='latest', device='cuda:0', seed=None):
    dataset_config = load_config(*loadpath, 'dataset_config.pkl')
    render_config = load_config(*loadpath, 'render_config.pkl')
    policy_model_config = load_config(*loadpath, 'policy_model_config.pkl')
    dynamic_model_config = load_config(*loadpath, 'dynamic_model_config.pkl')
    diffusion_config = load_config(*loadpath, 'diffusion_config.pkl')
    trainer_config = load_config(*loadpath, 'trainer_config.pkl')
    ...
    diffusion = diffusion_config(policy_model=policy_model, dynamic_model=dynamic_model)
    trainer = trainer_config(diffusion, dataset, renderer)
    ...
    return DiffusionExperiment(dataset, renderer, {"policy_model":policy_model, "dynamic_model":dynamic_model}, diffusion, trainer.ema_model, trainer, epoch)
